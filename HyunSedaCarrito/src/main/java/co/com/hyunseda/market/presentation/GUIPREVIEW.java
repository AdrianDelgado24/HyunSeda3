...
public class GUIPREVIEW extends javax.swing.JFrame implements Observer {
    ...
    private void btnRealizarPagoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnRealizarPagoActionPerformed
        ...
        double totalPago = Double.parseDouble(lblTotal.getText());
        if (totalPago == 0) {
            JOptionPane.showMessageDialog(null, "No ha seleccionado ningun producto.", "Mensaje", JOptionPane.INFORMATION_MESSAGE);
            return;
        }
        long codigoCuenta = Long.valueOf(txtCuenta.getText());
        Object selectedObject = cbxMetodoPago.getSelectedItem();
        String plataformaPago = selectedObject.toString();
        long payId = 1;
        long orderId = 1;

        while (payService.findPayById(payId) != null) {
            payId++;
           // orderId++;
        }
        
        while (orderService.findOrderById(orderId) != null) {
            orderId++;
        }
            Messages.showMessageDialog("order: "+ orderId, "Atención");
            Messages.showMessageDialog("pago: "+ payId, "Atención");
//      // && orderService.saveOrder(orderId, codigoCuenta, shoppingCartService.getCart().getCarro().size())
        if (payService.savePay(payId, totalPago, codigoCuenta, plataformaPago, orderId) && orderService.saveOrder(orderId, codigoCuenta, shoppingCartService.getCart().getCarro().size())) {
            Messages.showMessageDialog("Se grabó con éxito", "Atención");
             GUIREGISTRARUSUARIO abrir = new GUIREGISTRARUSUARIO(totalPago,categoryService,productService,shoppingCartService);
            //GUIREGISTRARUSUARIO abrir = new GUIREGISTRARUSUARIO();
